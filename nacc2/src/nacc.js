        ...
        if (numDays > 90) {
            ...
            for (let i = 24; i <= totalMonths - 12; i += 12) {
                ...
                if (this.showSpecialTags) {
                    if (comp === 60) {
                        specialTag = true;
                        this.createKeytag('15', isFace);
                    }
                    if (comp === 120) {
                        specialTag = true;
                        this.createKeytag('10', isFace);
                    }
                    if (comp === 180) {
                        specialTag = true;
                        this.createKeytag('16', isFace);
                    }
                    if (!specialTag && comp === 300) {
                        specialTag = true;
                        this.createKeytag('12', isFace);
                    }
                    if (!specialTag && comp === 360) {
                        specialTag = true;
                        this.createKeytag('14', isFace);
                    }
                    if (!specialTag && comp % 120 === 0) {
                        specialTag = true;
                        this.createKeytag('11', isFace);
                    }
                }
